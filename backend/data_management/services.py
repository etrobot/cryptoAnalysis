...
def run_news_evaluation_wrapper(task_id: str, top_n: int, news_per_symbol: int, openai_model: str, stop_event: Optional[threading.Event] = None):
    ...
    try:
        # Try to acquire semaphore with timeout to prevent blocking indefinitely
        if not ANALYSIS_SEMAPHORE.acquire(timeout=5):
            logger.warning(f"News evaluation task {task_id} could not acquire semaphore - another task is running")
            task = get_task(task_id)
            if task:
                task.status = TaskStatus.FAILED
                task.message = "无法启动新闻评估：已有其他任务正在运行，请等待完成后重试"
                task.completed_at = datetime.now().isoformat()
                from utils import bump_task_version
                bump_task_version(task_id)
            return
        ...
        semaphore_acquired = True
        logger.info(f"News evaluation task {task_id} acquired semaphore, starting execution")
        
        run_news_evaluation_task(task_id, top_n, news_per_symbol, openai_model, stop_event=stop_event)
    except Exception as e:
    ...
    if error_occurred:
        logger.error(f"News evaluation task {task_id} encountered an error and was marked as failed")


def create_news_evaluation_task(top_n: int = 10, news_per_symbol: int = 3, openai_model: str = "gpt-3.5-turbo") -> str:
    ...
    # Start background thread with error wrapper
    thread = threading.Thread(target=run_news_evaluation_wrapper, args=(task_id, top_n, news_per_symbol, openai_model, stop_event))
    thread.daemon = True
    TASK_THREADS[task_id] = thread
    thread.start()
